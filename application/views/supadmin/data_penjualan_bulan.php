                        ...
                        <div class="card-header">
                            <h4>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <?php foreach ($data_cabang as $dac) : ?>
                                                <a class="dropdown-item" href="<?= base_url('superadmin/laporan_penjualan_bulan') ?>/<?= $dac['id'] ?>"><?= $dac['nama_cabang'] ?></a>
                                            <?php endforeach; ?>
                                        </div>
                                        <?php if ($this->uri->segment(3)) : ?>
                                            <a href="<?= base_url('cetak/laporan_penjualan_bulan?idCabang=' . $this->uri->segment(3)) ?>" target="_blank" class="btn btn-sm btn-info mt-2"><i class="fas fa-print"></i></a>
                                            <a href="<?= base_url('export/excel_data_penjualan_bulan/' . $this->uri->segment(3)) ?>" target="_blank" class="btn btn-outline-warning btn-sm mt-2" title="Export Ke Excel"><i class="fas fa-file-excel"></i> Export Ke Excel</a>
                                        <?php else : ?>
                                            <a href="<?= base_url('cetak/laporan_penjualan_bulan?idCabang=1') ?>" target="_blank" class="btn btn-sm btn-info mt-2"><i class="fas fa-print"></i></a>
                                            <a href="<?= base_url('export/excel_data_penjualan_bulan/1') ?>" target="_blank" class="btn btn-outline-warning btn-sm mt-2" title="Export Ke Excel"><i class="fas fa-file-excel"></i> Export Ke Excel</a>
                                        <?php endif; ?>
                                ...
                                <?php elseif ($user['role_id'] == 2) :
                                    ...
                                    <a href="<?= base_url('cetak/laporan_penjualan_bulan?idCabang=' . $user['penempatan_cabang']) ?>" target="_blank" class="btn btn-sm btn-info mt-2"><i class="fas fa-print"></i></a>
                                    <button type="button" id="reloadData" class="btn btn-sm btn-outline-success mt-2"><i class="fas fa-redo-alt"></i> Refresh</button>
                                    <a href="<?= base_url('export/excel_data_penjualan_bulan/'.$user['penempatan_cabang']) ?>" target="_blank" class="btn btn-outline-warning btn-sm mt-2" title="Export Ke Excel"><i class="fas fa-file-excel"></i> Export Ke Excel</a>
    ...
    $(document).ready(function() {
        ...
        <?php if ($this->uri->segment(3)) : ?>
            $(document).on('click', '.btnSearch', function() {
                $.ajax({
                    url: "<?= $user['role_id'] == 1 ? base_url('superadmin/search_data_jual_bulan_cabang/' . $this->uri->segment(3)) : base_url('admin/search_history') ?>",
                    type: "POST",
            ...
            $(document).on('click', '#reloadData', function() {
                $('#disini').load("<?= $user['role_id'] == 1 ? base_url('superadmin/show_penjualan_bulan_cabang/' . $this->uri->segment(3)) : base_url('admin/show_history_penjualan') ?>");
            });
        <?php else : ?>
            $(document).on('click', '.btnSearch', function() {
                $.ajax({
                    url: "<?= $user['role_id'] == 1 ? base_url('superadmin/search_data_jual_bulan') : base_url('admin/search_data_jual_bulan') ?>",
                    type: "POST",
                    data: $('#searchDate').serialize(),
                    success: function(data) {
                        $('#disini').html(data);
            ...
            $(document).on('click', '#reloadData', function() {
                $('#disini').load("<?= $user['role_id'] == 1 ? base_url('superadmin/show_penjualan_bulan') : base_url('admin/show_penjualan_bulan') ?>");
            });
        <?php endif; ?>
